...
from uno.flux.util import (
    ...
    load_flow_model_only_lora_fuse,
    load_flow_model_quintized,
    load_flow_model_merge,
    load_t5,
)
...
class UNOPipeline:
    def __init__(
        ...
        if only_lora:
            if self.lora_path is not None:
                self.model = load_flow_model_only_lora_fuse(
                    model_type,
            ...
            self.model = load_flow_model_merge(model_type, device="cpu" if offload else self.device)
    ...
    def forward(
        ...
        # save_tensor({'x': x, 'timesteps': timesteps}, 'noise_and_timestep.pt', save_path)
        ...
        # save_tensor({'x_1_refs': x_1_refs}, 'x_1_refs.pt', save_path)
        ...
        # save_tensor({'ref_img': inp_cond['ref_img'][0]}, 'reference_img.pt', save_path)
        # save_tensor({'img_ids': inp_cond['img_ids']}, 'img_ids.pt', save_path)
        # save_tensor({'ref_img_ids': inp_cond['ref_img_ids']}, 'ref_img_ids.pt', save_path)
        # save_tensor({'txt_ids': inp_cond['txt_ids']}, 'txt_ids.pt', save_path)
        ...
        # save_tensor({'denoised_x': x}, 'denoised_x.pt', save_path)
